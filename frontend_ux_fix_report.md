...
27. **標注投票與回覆層級**：主標注改為 Reddit 式左側垂直票軌；淨分變化會依增減方向上下滾動。「加入這個討論串」固定在 modal 底部，不再混在回覆尾端。手機先顯示兩則回覆，並可展開或收起。
28. **評論與星等解耦**：文字評論不再要求或修改書籍星等，首頁及書房的評論表單都移除星星；書籍評分仍保留在書卡的獨立評分控制。
29. **書房社群密度**：「評論與標注」中的五類完整資料不刪減、不隨機抽樣，改放入固定高度且可獨立向下捲動的懸浮資料塊，避免所有內容把整頁無限拉長。
30. **書房三態顯示**：確認登入、未登入入口、已登入且資料載入完成改為互斥狀態；成功載入後 loading 與登入宣傳區一定隱藏，失敗時提供重新載入。
31. **互動通知同意**：保留五種通知偏好，新增「全部互動通知」總開關，完整開啟、部分開啟與全部關閉都有正確 checked／indeterminate 狀態。
32. **泡泡閾值即時同步**：拖曳時立即重新渲染泡泡並 debounce 回寫伺服器；登入者以伺服器值為準並同步本機，100% 可隱藏所有公開泡泡，私人標注不受影響。
33. **通知上限與刪除**：通知 API、摘要與書房只提供最新 30 則；通知卡新增鍵盤可操作的開啟與刪除按鈕，刪除仍由 user_id 與 RLS 雙重限制。
34. **館藏來源首次揭露**：`#source-info` 改為低飽和卷宗／舊報刊視覺，以 IntersectionObserver 首次進場依序淡入；手機統一由下往上、reduced-motion 直接顯示，未載入 JavaScript 時內容預設可見。
35. **Realtime 投票動畫收斂**：標注、標注回覆與讀者回饋以樂觀更新先播放一次分數滾動；WebSocket 回補資料只有在伺服器淨分確實不同時才再播放，不會把同一票重複計算或重播。
36. **讀者回饋一致互動**：回饋主題與回覆同樣採左側垂直票軌，modal 內容獨立捲動、輸入框固定底部；Realtime 更新保留 scrollTop，並以低干擾狀態提示同步中的新動態。
37. **評論完整浮窗**：讀者評論 modal 取消三則抽樣，完整評論串在內部捲動；上方補入封面、書名、作者、簡介與閱讀連結，下方評論輸入框固定，隨時可以發表或更新評論。
38. **書房登入入口確實互斥**：補上 `[hidden]` 的明確 CSS 規則，避免元件自身的 `display:grid` 蓋過 hidden；相同登入者已有資料時立即顯示內容並在背景更新，不再重新遮回 loading。
39. **統一社群投票視覺**：標注、標注回覆、讀者回饋與書評讚賞共用同一組左側垂直票軌、按鈕狀態與數字樣式；書評只有讚賞，因此保留單向按鈕但位置、形狀與色彩回饋一致。
40. **可靠分數滾動與卡片換位**：將會被重繪中斷的 CSS class 實作改為共用 JavaScript，確保 420ms 分數動畫完整播放；增加時數字向上、減少時向下。依淨分排序改變時以 FLIP 保留前後座標，卡片在 1000ms 內沿軌跡交換位置，不使用淡入淡出，並維持 modal 的 `scrollTop`。
