...
27. **標注投票與回覆層級**：主標注改為 Reddit 式左側垂直票軌；「加入這個討論串」成為獨立、捲動時固定的輸入區，不再混在回覆尾端。手機先顯示兩則回覆，需要時再展開完整討論。
28. **評論與星等解耦**：文字評論不再要求或修改書籍星等，首頁及書房的評論表單都移除星星；書籍評分仍保留在書卡的獨立評分控制。
29. **手機社群密度**：評論浮窗在手機穩定抽樣三則並提供查看全部；每章公開標注泡泡最多顯示四個穩定樣本，點擊後仍進入完整浮動討論窗，避免每次渲染隨機跳動。
