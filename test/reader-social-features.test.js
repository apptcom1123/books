...
test("reader feedback is a standalone searchable thread surface", () => {
  ...
  const motion = read("public/social-motion.js");
  const motionCss = read("public/social-motion.css");
  ...
  assert.match(script, /social-vote-rail/);
  assert.match(motionCss, /\.social-vote-rail/);
  assert.match(motion, /function animateScores/);
  assert.match(motion, /function animateCardSwap/);
  assert.match(motion, /SCORE_DURATION = 420/);
  assert.match(motion, /CARD_SWAP_DURATION = 1000/);
  assert.doesNotMatch(motion, /opacity/);
  assert.match(script, /votePending\.size \? 1050 : 280/);
...
test("annotation bubbles aggregate five-position local threads with ranked realtime replies", () => {
  ...
  assert.match(motion, /translateY\(-50%\)/);
  assert.match(motion, /CARD_SWAP_DURATION = 1000/);
  assert.match(script, /annotationMutationPending\.size \? 1050 : 280/);
...
test("text reviews stay separate from ratings and use a scrollable book-aware dialog", () => {
  ...
  assert.match(app, /social-vote-rail like-only/);
  assert.match(app, /queueReviewLikeAnimation/);
  assert.match(app, /animateCardSwap/);
  assert.match(app, /reviewMutationPending\.size \? 1050 : 320/);
  assert.match(motion, /duration:CARD_SWAP_DURATION/);
