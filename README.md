...
閱讀器不會在反白時立刻打開表單。完成選取後先按畫面下方的「增加標注」，再決定內容與公開範圍。公私標注都儲存在 Supabase；RLS 只允許作者讀取自己的私人標注。和 `yz_json` 一樣，標注以章節內的文字起始位置計算，每 5 個位置聚合成一顆泡泡；公私標注分開聚合，舊版沒有文字位移的資料保持獨立以避免錯誤合併。公開泡泡依局部討論串的淨分、讚數與時間排名，顯示閾值可在閱讀器或個人書房調整。點兩下泡泡會開啟該位置的標注討論；每個標注獨立成頁，可用上一個／下一個按鈕或左右滑動切換。

## 個人書房與通知

登入後從右上角或「我的書房」進入 `/account.html`，可管理收藏、閱讀進度、評分、書評、自己的標注與回覆、收藏的公開標注／讀者評論，以及通知偏好。書籍星等與文字評論是兩個獨立操作，發表或編輯評論不會改動評分。公開標注被收藏／按讚、標注收到回覆、書評被按讚／收藏、留言收到回覆時會產生站內通知；自己的互動不會通知自己。書房只顯示最新 30 則通知，使用者可以個別刪除。
